...
// made for: ICS3U
// pong CPT
//song link  https://www.bensound.com/royalty-free-music

import PlaygroundSupport
import SpriteKit
import AVFoundation

var SPEED : Double = 0.60
var ballxSpeed : Int = -8
var ballySpeed : Int = -3
var gameLevel : Int = 1
...
class instructionsScene: SKScene, SKPhysicsContactDelegate {
    ...
    let thridLineLabel : UILabel = UILabel()
    ...
    override func didMove(to view: SKView) {
        ...
        thridLineLabel.text = "https://www.bensound.com/royalty-free-music"
        thridLineLabel.textColor =  _ColorLiteralType(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0)
        view.addSubview(thridLineLabel)
        thridLineLabel.translatesAutoresizingMaskIntoConstraints = false
        thridLineLabel.topAnchor.constraint(equalTo: view.topAnchor, constant: 500).isActive = true
        thridLineLabel.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 400).isActive = true
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let nodeTouchedName = nodeTouched.name{
            ...
            if nodeTouchedName == "Audio" {
                // plays music
                self.scene?.run(SKAction.repeatForever(SKAction.playSoundFileNamed ("Background Music.mp3", waitForCompletion: true)))
            }
...
// This function makes the bot paddle more difficult to beat
// as the levels increase.
func setBotPaddleLevel() {
    ...
    {
        SPEED = SPEED - (0.10/Double(gameLevel))
    }
    
    // don't want this value going negative
    if (SPEED <= 0)
    {
        SPEED = 0.01
    }
...
class normalGameScene: SKScene , SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        // set the bot paddle level
        setBotPaddleLevel()
        
        ballxSpeed = -8 - (gameLevel*2)
        ballySpeed = -3 - (gameLevel*1)
        ...
        ball.physicsBody?.applyImpulse(CGVector(dx: ballxSpeed, dy: ballySpeed))
    ...
    override func  update(_ currentTime: TimeInterval) {
        ...
        if ball.position.x > botPaddle.position.x + 20 {
            playerPaddle.position = CGPoint(x: 100 , y: (frame.size.height / 2))
